...
# Betas del modelo en formato tidy (word-topic probabilities) -----
...
# Comparación de betas entre topics
...
beta_spread %>% 
  top_n(20, wt = abs(log_ratio)) %>% 
  dplyr::arrange(log_ratio) %>% 
  ggplot(aes(reorder(term, log_ratio), log_ratio, fill = log_ratio < 0)) +
  geom_bar(stat = 'identity') +
  coord_flip()
  
  
# Gammas del modelo en formato tidy (document-topic probabilities) --------

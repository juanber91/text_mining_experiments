...
custom_stop_words <- bind_rows(data_frame(word = c("miss", 'harry', 'ron',
                                                   'hermione', 'dumbledore'), 
                                          lexicon = c("custom")), 
                               stop_words)
...
rowlingsentiment <- tidy_books %>%
  inner_join(get_sentiments("bing")) %>%
  count(book, index = linenumber %/% 80, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)

ggplot(rowlingsentiment, aes(index, sentiment, fill = book)) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~book, ncol = 2, scales = "free_x")

# Comparamos diccionarios para un libro específico ---------
pride_prejudice <- tidy_books %>% 
  filter(book == "Harry Potter and the Prisoner of Azkaban")
...
tidy_books %>%
  ...
  count(word) %>%
  with(wordcloud(word, n, random.order = F, colors = 'blue', max.words = 200))
